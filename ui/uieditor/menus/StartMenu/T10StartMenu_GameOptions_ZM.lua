require( "ui.uieditor.menus.StartMenu.T10StartMenu_Options" )
require( "ui.uieditor.widgets.Lobby.Common.T10ButtonListItem" )
...
local ToggleThirdPerson = function ( self, element, controller, actionParam, menu )
	...
	Engine.SendMenuResponse( self, "StartMenu_Main", "ui_menu_option_third_person|" .. ( newValue and "1" or "0" ) )
	...
	if CoD.isZombie then
		...
		table.insert( options, {
			models = {
				...
				action = function ( self, element, controller, actionParam, menu )
					NavigateToMenu( menu, "T10StartMenu_Options", true, controller )
				end
			}
		} )
...
CoD.T10StartMenu_GameOptions_ZM = InheritFrom( LUI.UIElement )
CoD.T10StartMenu_GameOptions_ZM.new = function ( menu, controller )
	...
	self:setClass( CoD.T10StartMenu_GameOptions_ZM )
	self.id = "T10StartMenu_GameOptions_ZM"
	...
	self.buttonList = LUI.UIList.new( menu, controller, 11, 0, nil, true, false, 0, 0, false, false )
	self.buttonList:makeFocusable()
	self.buttonList:setLeftRight( true, false, 0, 0 )
	self.buttonList:setTopBottom( true, false, 0, 0 )
	self.buttonList:setWidgetType( CoD.T10ButtonListItem )
